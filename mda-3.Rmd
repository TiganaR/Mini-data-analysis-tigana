...
1. There is obviously strong relationships between time of year (`month` and `season`), `extreme_type` and `flow`, but is one significantly better for predicting flow?

2. Is the variation in flow year to year associated with the month in which flow was collected?
<!----------------------------------------------------------------------------->
...
**Research Question**: There is obviously strong relationships between time of year (`month` and `season`), `extreme_type` and `flow`, but is one significantly better for predicting flow?

There is a significant flow average difference among the seasons?
...
```{r}
anova_model <- aov(flow ~ season, data=flow_sample)
summary(anova_model)
```

```{r}
ggplot(data=flow_sample, aes(x=season, y=flow, fill=season)) +
...
```{r}
linear_model <- lm(flow ~ season, data=flow_sample)
...
# ?write.csv
# write.csv(my_table_R_object, "my_path/table.csv")
```
